...
public:
    ...
    void setParams(std::map<int,int> markersConfiguration, general_marker_detector::GeneralMarkerDetector *detector, int w,int h,const aruco::MarkerMap &mmap,float f=1.5){
        ...
        for(auto m:mmap)
                marker_points[m.id]=getMarkerIdPcd(markersConfiguration, detector, m, 0.5);
        sgl::Matrix44 cam;
        cam.translate({0,4,0});
    ...
    void draw(cv::Mat &image,bool showNumbers,cv::Mat camerPose=cv::Mat()){
        ...
        auto drawMarker=[](sgl::Scene &Scn, const aruco::Marker3DInfo &m , int width){
            ...
            */
            ...
            int rcolor = 0;
            int stepcolor = 80;
            for(uint i = 0; i < m.points.size(); i++){
                gcolor = stepcolor*i; if(gcolor > 255){
                    gcolor = 255;
                    rcolor = stepcolor*i-gcolor;
                    if(rcolor > 255){
                        rcolor = 255;
                    }
                    if(rcolor < 0){
...
private:
    ...
    std::vector<sgl::Point3> getMarkerIdPcd(std::map<int,int> markersConfiguration, general_marker_detector::GeneralMarkerDetector *detector, aruco::Marker3DInfo &minfo, float perct=1 )
    {
        ...
        // OLD -> auto orgPoints=aruco::Marker::get3DPoints( minfo.getMarkerSize());
        // old -> auto orgPoints = detector->getMarkers3DCornersSchema(minfo.getMarkerSize());
        //auto orgPoints = detector->getMarkers3DCornersSchema(minfo.getMarkerSize(), cv::Mat(), detector->getMarkerConfigById(minfo.id));
        auto orgPoints = detector->getMarkers3DCornersSchema(minfo.getMarkerSize(), cv::Mat(), markersConfiguration[minfo.id]);
 ...
 public:
    ...
    void setParams(std::map<int,int> markersConfiguration, general_marker_detector::GeneralMarkerDetector *detector, aruco::MarkerMap &mmap,float f,int width,int height,std::string wname){
        ...
        mapdrawer.setParams(markersConfiguration, detector, _w,_h,mmap,_f);
