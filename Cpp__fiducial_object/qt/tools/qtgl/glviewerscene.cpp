...
void GlViewerScene::drawBackground(QPainter *painter, const QRectF &)
{
    ...
    float f[16]={_scale,0,0,0, 0,_scale,0,0, 0,0,_scale,0, 0,0,0,1};
   glMultMatrixf( f );
   glPushMatrix();
    ...
    ///////////////////////////
    if(_AxisSize>0) drawAxis(_AxisSize);
...
/**
  */
void GlViewerScene::drawAxis(float limitAxe)
{
  float limit = 10.0f;
...
bool GlViewerScene::getNearestProjection(uint32_t px, uint32_t py, float &outx, float &outy, float &outz , int maxWsize){
    ...
    auto getNearestValidElement=[](int x,int y,int w,int h,GLint *viewport,std::vector<float> &data){
