...
public:
    ...
    general_marker_detector::GeneralMarkerDetector *detector;
    inline FullSceneOptimizer(general_marker_detector::GeneralMarkerDetector *newdetector = NULL){detector = newdetector; };

    struct Params{
        int max_iters=200; // OLD -> 200
        double min_error=0.1; // OLD -> 0.1
        std::set<uint32_t> fixedMarkers;
        std::set<uint32_t> fixedFrames;
        bool fix_camera_params=false;
        bool verbose=false;
        double min_step_error=1; // 1
    ...
    inline void setConfigsByMarkers(std::map<int,int> markersConfigurations){
      for(auto conf: markersConfigurations)
        this->_markersConfiguration[conf.first] = conf.second;
    };
