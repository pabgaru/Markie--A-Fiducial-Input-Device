...
double FullSceneOptimizer::optimize(MarkerSet & MarkerSet, FrameSet & FrameSet,aruco::CameraParameters &cam_params, Params &params){
   ...
   // cout<<"INI SOLUTION: ";for(int i=0;i<10;i++)cout<<sol(i)<<" ";cout<<endl;
    double minError=0.001;( float(error.size())/2.)*sba_ba_params.min_error;
    double minStepError=1;( float(error.size())/2.)*sba_ba_params.min_step_error;
// cout<<"MIN STEP ERROR="<<minStepError<<endl;
  ...
  //  solver.solve(sol,std::bind(&FullSceneOptimizer::error_optimization,this,std::placeholders::_1,std::placeholders::_2),std::bind(&FullSceneOptimizer::error_jacobian,this,std::placeholders::_1,std::placeholders::_2));
    solver.solve(sol,std::bind(&FullSceneOptimizer::error_optimization,this,std::placeholders::_1,std::placeholders::_2));//,std::bind(&FullSceneOptimizer::error_jacobian,this,std::placeholders::_1,std::placeholders::_2));
      ...
      cout<<"ERRXX FINAL="<<error.cwiseProduct(error).sum()<<endl;
      stats(error);
...
//            // OLD -> markers_points3d.insert( std::make_pair(marker->second.id, getMarkerPoints(marker->second.ssize,aruco_mm::getRTMatrix(r,t))));
//            markers_points3d.insert( std::make_pair(marker->second.id, this->detector->getMarkers3DCornersSchema(marker->second.ssize,aruco_mm::getRTMatrix(r,t))));
...
//    for(auto frame: sba_frameSet) errorvectorsize+=frame.second.markers.size()*(this->detector->getMarkers3DCornersSchema().size()*2);
...
void FullSceneOptimizer::error_optimization(const SparseLevMarq<double>::eVector &sol,SparseLevMarq<double>::eVector &error)
{
    ...
    std::unordered_map<uint32_t,vector<cv::Point3f> > markers_points3d;
    {
        ...
        for(size_t i=0;i<sba_markerSet.size();i++,marker++){
            ...
            double sizeToUse = marker->second.ssize;

            markers_points3d.insert(
                 std::make_pair(
                     marker->second.id,
                     this->detector->getMarkers3DCornersSchema(sizeToUse, aruco_mm::getRTMatrix(r,t), this->getConfigFromMarkerId(marker->second.id) )
                 )
            );
    ...
    //for(auto frame: sba_frameSet) errorvectorsize+=frame.second.markers.size()*8;//each markers provides 4 points, with two measures each(x,y)
    //for(auto frame: sba_frameSet){
    //    errorvectorsize+=frame.second.markers.size()*(this->detector->getMarkers3DCornersSchema().size()*2);
    //}
    for(auto frame: sba_frameSet){
        //errorvectorsize += frame.second.markers.size() * (this->detector->getMarkers3DCornersSchema().size()*2);
        for(auto marker: frame.second.markers){
            errorvectorsize += (marker.getCorners3D().size()*2);
        }
    }
    ...
    vector<int> frame_start;
    {
        ...
        for(auto frame:sba_frameSet ){
            ...
            //foffset+=frame.second.markers.size()*(this->detector->getMarkers3DCornersSchema().size()*2);
            for(auto marker: frame.second.markers){
                foffset += (marker.getCorners3D().size()*2);
            }
...
//        int frameOffset=6*sba_markerSet.size()+curFrameIdx*6;
        int frameOffset=0;
        for(auto m: sba_markerSet){
            frameOffset += 6;
        }
        frameOffset += curFrameIdx*6;
        ...
        //project all these points
        cv::projectPoints(m_points3d,r,t,camera,dist,m_points2d);
        //        //add the 2dpoints to the output vector
        int currErrRow=frame_start[curFrameIdx];
        for(size_t  p=0;p<m_points2d.size();p++,currErrRow+=2) {
            double ex=m_points2d[p].x-m_points2d_observed[p].x;
    ...
    for(int markerIdx=0;markerIdx<int(sba_markerSet.size());markerIdx++){
        ...
        if (sba_ba_params.fixedMarkers.find( marker.id )==sba_ba_params.fixedMarkers.end() ){//The fixd markers derivatives are set of 0 der to avoid moving them
            ...
            for(int rt=0;rt<6;rt++){
                ...
                // OLD -> auto obj_points_p_n=getMarkerPoints(marker.ssize,getRTMatrix(rt_p));
                // OLD -> auto obj_points_n=getMarkerPoints(marker.ssize,getRTMatrix(rt_n));
                // old -> auto obj_points_p_n= this->detector->getMarkers3DCornersSchema(marker.ssize,getRTMatrix(rt_p));
                // old -> auto obj_points_n= this->detector->getMarkers3DCornersSchema(marker.ssize,getRTMatrix(rt_n));
                auto obj_points_p_n= this->detector->getMarkers3DCornersSchema(marker.ssize,getRTMatrix(rt_p), this->getConfigFromMarkerId(marker.id));
                auto obj_points_n= this->detector->getMarkers3DCornersSchema(marker.ssize,getRTMatrix(rt_n), this->getConfigFromMarkerId(marker.id));
                ...
                for(auto mfp: mfv ){
                    ...
                    // OLD -> int numOfPoints = 4;
                    // old -> int numOfPoints = this->detector->getMarkers3DCornersSchema().size(); // sba_markerSet[0].size(); std::cout<<"numOfPoints: "<<numOfPoints<<std::endl;
                    int numOfPoints = this->detector->getMarkers3DCornersSchema(-1,cv::Mat(),this->getConfigFromMarkerId(marker.id)).size(); // sba_markerSet[0].size(); std::cout<<"numOfPoints: "<<numOfPoints<<std::endl;

                    for(int p=0;p<numOfPoints;p++){
                        double derv_errx=(img_points_p_n[p].x-img_points_p_n[p+numOfPoints].x)/(2.*der_epsilon);
                        jac_omp[omp_thread].push_back(Eigen::Triplet<double>( errorRow++,markerIdx*6+rt,derv_errx));
                        double derv_erry=(img_points_p_n[p].y-img_points_p_n[p+numOfPoints].y)/(2.*der_epsilon);
