...
bool GlobalGraphMarkerMapper::process (const markerSet &in_detected_markers,int imageIdx ,bool forceConnectedComponent ) {
    ...
    for(const auto &marker:detected_markers){
        ...
        // OLD -> cv::Mat mpose=solvePnP(this->getGeneralMarkerDetector().getMarkers3DCornersSchema(), marker, _cam_params);
        //cv::Mat mpose=solvePnP(this->getGeneralMarkerDetector().getMinimum4Corners3DSchema(), marker.getAsMinimun4Corners2D(), _cam_params);
        cv::Mat mpose=solvePnP(marker.getAsMinimun4Corners3D(), marker.getAsMinimun4Corners2D(), _cam_params);

        se3 pose=mpose;
        //ms[marker.id]=MarkerInfo(marker.id,mpose,_defaultMarkerSize); ms[marker.id].setCorners3D(this->getGeneralMarkerDetector().getMarkers3DCornersSchema()); // ms[marker.id].setCorners3D(marker.getCorners3D());
        ms[marker.id]=MarkerInfo(marker.id,mpose,_defaultMarkerSize); ms[marker.id].setCorners3D(marker.getCorners3D());
        ...
        bool angle_too_high = false; 
        {
            ...
            if (cameraMarkerDotProduct(mpose)>cos_limit){
              angle_too_high=true; //45deg // OLD ->>> angle_too_high=true;
            }
        }
...
void GlobalGraphMarkerMapper::optimize_impl (   ) {
 ...
 // finalize_();
    ...
    if (debug::Debug::getLevel()>=10) savePCDFile(this->_markersConfiguration, &this->getGeneralMarkerDetector(), "ss2.pcd",_markerSet);
    ...
    if (debug::Debug::getLevel()>=10) savePCDFile(this->_markersConfiguration, &this->getGeneralMarkerDetector(),  "outxx.pcd",_markerSet);
    ...
    aruco_mm::FullSceneOptimizer opt(&this->getGeneralMarkerDetector());

    // Markers - config
    opt.setConfigsByMarkers(this->_markersConfiguration);

    params.fixedMarkers={uint32_t(_originMarkerId)};
    params.max_iters=100;// Previous version 2/2/2022 --> 100
    params.min_error=0.05; // Previous version 2/2/2022 --> 0.5
    params.min_step_error=0.005;// Previous version 2/2/2022 --> 0.05
    params.verbose=true;
    ...
    if (debug::Debug::getLevel()>=10) savePCDFile(this->_markersConfiguration, &this->getGeneralMarkerDetector(),  "outxx2.pcd",_markerSet);
    ...
    for(int fidx=0;fidx<int(fs.size());fidx++){
      ...
      //  cerr<<frame.frame_idx<<" ";
        ...
        for(size_t m=0;m< frame.markers.size();m++ ){
            ...
            // OLD -> auto poses=IPPE::solvePnP_(getMarkerPoints(_defaultMarkerSize),marker,cam_params.CameraMatrix,cam_params.Distorsion);
            // old -> auto poses=IPPE::solvePnP_(marker.getCorners3D(),marker,cam_params.CameraMatrix,cam_params.Distorsion);
            // OLD -> auto poses=IPPE::solvePnP_(this->getGeneralMarkerDetector().getMarkers3DCornersSchema(),marker,cam_params.CameraMatrix,cam_params.Distorsion);
            //auto poses=IPPE::solvePnP_(this->getGeneralMarkerDetector().getMinimum4Corners3DSchema(),marker.getAsMinimun4Corners2D(),cam_params.CameraMatrix,cam_params.Distorsion);
            auto poses=IPPE::solvePnP_(marker.getAsMinimun4Corners3D(),marker.getAsMinimun4Corners2D(),cam_params.CameraMatrix,cam_params.Distorsion);


            if (poses[0].second/poses[1].second< this->getGeneralMarkerDetector().getEstimatedPosesDifferenceThreshold() ) { // OLD -> if (poses[0].second/poses[1].second<0.6) {
...
//            err+=cv::norm (pp-pd_und[i]);
    ...
    // OLD -> auto marker_points=getMarkerPoints(_defaultMarkerSize);

    //auto marker_points = this->getGeneralMarkerDetector().getMarkers3DCornersSchema(_defaultMarkerSize);// fs[0].markers[0].getCorners3D();
            ...
            if ( graph(i,j).size()>=size_t(min_num_edges_between_nodes)){//require at least min_num_edges_between_nodes views to consider the link
                ...
                for(auto pose_frame:graph(i,j)){
                    ...
                    for(auto pf_t:graph(i,j)){
                        ...
                        // Multimarker change
                        // Old: err+=get_rpj_err(marker_points,frame_undpoints[pf_t.frame],c2m,cam_params.CameraMatrix);
                        err+=get_rpj_err(fs[pf_t.frame].markers.get(mi_id).getCorners3D(),frame_undpoints[pf_t.frame],c2m,cam_params.CameraMatrix);
...
FrameSet GlobalGraphMarkerMapper::getOptimizingFrameSet(FrameSet &fset, MarkerSet &mset, aruco::CameraParameters cam_params, double maxRerpjErr, int minNMarkers){
    ...
    // OLD -> auto marker_points=getMarkerPoints(_defaultMarkerSize);
    // OLD -> auto marker_points = this->getGeneralMarkerDetector().getMarkers3DCornersSchema();
    //auto marker_points = this->getGeneralMarkerDetector().getMinimum4Corners3DSchema();
    ...
    for(int i=0;i<int(fset.size());i++){
        ...
        if (int(used_markers.size())>=minNMarkers){
            ...
            for(auto marker:used_markers){
                ...
                // OLD -> auto poses=IPPE::solvePnP(marker_points,marker,cam_params.CameraMatrix,cam_params.Distorsion);
                //auto poses=IPPE::solvePnP(marker_points,marker.getAsMinimun4Corners2D(),cam_params.CameraMatrix,cam_params.Distorsion);
                auto poses=IPPE::solvePnP(marker.getAsMinimun4Corners3D(),marker.getAsMinimun4Corners2D(),cam_params.CameraMatrix,cam_params.Distorsion);
...
MarkerSet GlobalGraphMarkerMapper::calculateMarkerLocationsFromExpansionGraph(graph::Floyd & falgo,  StgMatrix<se3> &poseGraph, uint32_t origin_id,std::map<uint32_t,uint32_t> &markers_id_pos,std::map<uint32_t,uint32_t> &markers_pos_id)
{
    ...
    //MarkerInfo info(origin_id,cv::Mat::eye(4,4,CV_32F),_defaultMarkerSize);  info.setCorners3D(this->detected_markers[0].getCorners3D());
    MarkerInfo info(origin_id,cv::Mat::eye(4,4,CV_32F),_defaultMarkerSize);  info.setCorners3D(this->getGeneralMarkerDetector().getMarkers3DCornersSchema(-1, cv::Mat(), this->getConfigFromMarkerId(origin_id)));
    ...
    for(auto m:markers_id_pos){
        if (m.first!=origin_id){
            ...
            //mset[ m.first]=MarkerInfo(m.first,RT.inv(), _defaultMarkerSize);  mset[ m.first].setCorners3D(this->detected_markers[0].getCorners3D());
            mset[ m.first]=MarkerInfo(m.first,RT.inv(), _defaultMarkerSize);  mset[ m.first].setCorners3D(this->getGeneralMarkerDetector().getMarkers3DCornersSchema(-1, cv::Mat(), this->getConfigFromMarkerId(m.first)));
