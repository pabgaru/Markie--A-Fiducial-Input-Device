...
public:
    ...
    void optimize_impl (    ) ;
    bool process (const markerSet &in_markers, int imageIdx=-1  , bool forceConnectedComponent=false) ;
...
private:
    ...
    float max_marker_cam_angle = 75; //degs // OLD -> 75
    float max_angular_dot_value = 0.15; // OLD -> 0.15
    float radius_search_t = 0.1; // OLD -> 0.1  //translation radius search in kdtree
    std::map<uint32_t,Kdtree> kdtrees;//for each markers, a kdtree with the observed ,stored locations
    markerSet detected_markers;
