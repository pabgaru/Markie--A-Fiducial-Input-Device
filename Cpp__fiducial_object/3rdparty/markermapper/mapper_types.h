...
/**
 * @brief The markerSet struct represents the set of markers found in a image. It represents the 2d projections and their ids
 */
struct MARKERMAPPER_API markerSet: public std::vector<general_marker::GeneralMarker>{


    markerSet(){}
    markerSet(const std::vector<general_marker::GeneralMarker> &ms):std::vector<general_marker::GeneralMarker>(ms){}
...
struct MARKERMAPPER_API FrameInfo {
    ...
    markerSet markers;//set of markers detected by aruco
    int frame_idx;//index of frame in sequence
    FrameInfo(){}
    FrameInfo(int fidx){frame_idx=fidx;};
    FrameInfo(int fidx,const markerSet &ams){frame_idx=fidx;markers=ams;}
    FrameInfo(int fidx,const markerSet &ams,const cv::Mat &RT){frame_idx=fidx;markers=ams;rt_c2g=RT.clone();}
...
struct MARKERMAPPER_API MarkerInfo: public general_marker::GeneralMarker {

    inline MarkerInfo():general_marker::GeneralMarker(){};
    inline MarkerInfo(int id, cv::Mat rt, float size):general_marker::GeneralMarker(id, rt, size){};
...
struct MARKERMAPPER_API IoHelper{
    ...
    static void toStream(const general_marker::GeneralMarker &m,std::ostream &str);
    static void fromStream(  general_marker::GeneralMarker &m,std::istream &str);
