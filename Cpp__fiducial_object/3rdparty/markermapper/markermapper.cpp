...
//        cv::imshow("Frame", image); cv::waitKey();

        markerSet markers = _mdetector->detect(image);
        size_t size = markers.size();
        markermapper_debug_msg("   Num of markers detected: " <<size,5);

        // if (markers.size()<2)return;


        // Insert markers - configurations
        for(auto marker: markers){
            this->setConfigByMarkerId(marker.getId(), marker.getConfig());
        }
    ...
    bool MarkerMapper::isOptimizationFinished(){ //only when call finalize in non-blocking mode
        return _finishedProcessing;
    }

    void MarkerMapper::waitForOptimizationFinished(){ //waits until optimization is finished. Call when isOptimizationFinished()==true non-blocking mode
    ...
    markerSet MarkerMapper::deleteAllBut( markerSet &in,const std::set<uint32_t> &tokeep,const std::set<uint32_t> &tokeep2){
    ...
    std::vector<double> MarkerMapper::getRprjErr(const FrameInfo &fi,    MarkerSet &mset, const aruco::CameraParameters &cam)
    {
        ...
        for(auto marker: fi.markers){
            if (mset.find(marker.id)!=mset.end()){
                ...
                // OLD -> cv::projectPoints(getMarkerPoints(mset[marker.id].ssize,mset[marker.id].rt_g2m),r,t,cam.CameraMatrix,cam.Distorsion,reprj_2d);

                // old ->
                cv::projectPoints(marker.getCorners3D(),r,t,cam.CameraMatrix,cam.Distorsion,reprj_2d);
                //cv::projectPoints(this->getGeneralMarkerDetector().getMarkers3DCornersSchema(),r,t,cam.CameraMatrix,cam.Distorsion,reprj_2d);
    ...
    double MarkerMapper::bundleAdjustment(){
        FullSceneOptimizer opt(&this->getGeneralMarkerDetector());
    ...
    ostream & operator<<(ostream &str,const MarkerInfo &fi){
        str<<fi.id<<" "<<fi.rt_g2m<<" "<<fi.ssize<<endl;
        return str;
    }
    ...
    void MarkerMapper::saveToPcd(string fpath,bool addViewLocations) {
        if (addViewLocations)
            savePCDFile(this->_markersConfiguration, &this->getGeneralMarkerDetector(), fpath,_markerSet,_frameSet,_originMarkerId);
        else
            savePCDFile(this->_markersConfiguration, &this->getGeneralMarkerDetector(), fpath,_markerSet,_originMarkerId);
    }
    ...
    aruco::MarkerMap MarkerMapper::getMarkerMap()  {
        ...
        mmap.setDictionary(_mdetector->getMarkerLabelerDict()); // mmap.setDictionary(_mdetector->getMarkerLabeler()->getName());
