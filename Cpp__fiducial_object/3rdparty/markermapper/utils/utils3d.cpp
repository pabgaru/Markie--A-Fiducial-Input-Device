...
vector<cv::Vec4f> getMarkerIdPcd(general_marker_detector::GeneralMarkerDetector *detector, int id,float ssize,cv::Mat rt_g2m,cv::Scalar color ){
   ...
   for(int y=0;y<img.rows;y++){
       for(int x=0;x<img.cols;x++){
           if (img.at<uchar>(y,x)!=0){
               points_id.push_back( cv::Point3f((float(x)/float(img.cols))-0.5,(float(img.rows-y)/float(img.rows))-0.5,0));
           }
       }
   }
   ...
   //now,scale
   for(auto &p:points_id)p*=ssize_2;
   //finally, translate
...
void  savePCDFile(std::map<int, int> markersConfiguration, general_marker_detector::GeneralMarkerDetector *detector, string fpath, const MarkerSet &ms, int origin) {
    (void)origin;
    ...
    for(auto m:ms)
    {
        ...
        // OLD -> auto points4=getPcdPoints(getMarkerPoints(m.second.ssize,m.second.rt_g2m),color);
        // old -> auto points4 = getPcdPoints( detector->getMarkers3DCornersSchema(m.second.ssize,m.second.rt_g2m) ,color);
        auto points4 = getPcdPoints( detector->getMarkers3DCornersSchema(m.second.ssize,m.second.rt_g2m, markersConfiguration[m.second.id]) ,color);


        points2write.insert(points2write.end(),points4.begin(),points4.end());


        auto points_id = getMarkerIdPcd(detector, m.second.id,m.second.ssize,m.second.rt_g2m,color);
...
void savePCDFile(string fpath,const MarkerSet &ms, const vector<cv::Mat >  &vector_rt_c2g,int org_id ) {
    ...
    for(auto m:ms)
    {
        ...
        auto points4=getPcdPoints(getMarkerPoints(m.second.ssize,m.second.rt_g2m),color);
        points2write.insert(points2write.end(),points4.begin(),points4.end());
        auto points_id=getMarkerIdPcd(m.second.id,m.second.ssize,m.second.rt_g2m,color);
        points2write.insert(points2write.end(),points_id.begin(),points_id.end());
        max_msize=std::max(max_msize,m.second.ssize);
    }
...
void savePCDFile(std::map<int, int> markersConfiguration, general_marker_detector::GeneralMarkerDetector *detector, string fpath,const MarkerSet &ms, const FrameSet &fset,int org_id,bool writeFrameIds,cv::Scalar marker_color,cv::Scalar frame_color) {
    std::vector<cv::Vec4f> points2write;
    float max_msize=-1;
    for(auto m:ms)
    {
    ...
    for(auto frame:fset){
        if (!frame.second.rt_c2g.empty()){
            ...
            // OLD -> auto pcam=getPcdPoints(getMarkerPoints(max_msize/2,g2c),frame_color,25);
            auto pcam=getPcdPoints( detector->getMarkers3DCornersSchema(max_msize/2,g2c) ,frame_color,25);

            points2write.insert(points2write.end(),pcam.begin(),pcam.end());

            if (writeFrameIds){
                pcam=getMarkerIdPcd(detector, frame.first,max_msize/2,frame.second.rt_c2g.inv(),cv::Scalar(255-frame_color[0],255-frame_color[1],255-frame_color[2]));//getCameraPoints(frame.second.rt_c2g,0.2);
                points2write.insert(points2write.end(),pcam.begin(),pcam.end());
            }
        }
    }
...
std::vector<cv::Point3f>   getMarkerPoints( const MarkerInfo &m){
    return getMarkerPoints( m.ssize,m.rt_g2m );
}
...
cv::Mat  solvePnP(const  vector<general_marker::GeneralMarker> &markers,    MarkerSet & MarkerSet,const aruco::CameraParameters &camp,bool ransac,cv::Mat initial_rt){
    vector<cv::Point2f> p2d;
    ...
    for(size_t i=0;i<markers.size();i++){
        if ( MarkerSet.find(markers[i].id)!=MarkerSet.end()){
            ...
            //auto m3d=getMarkerPoints(MarkerSet[markers[i].id].ssize,MarkerSet[markers[i].id].rt_g2m);
            auto m3d = MarkerSet[markers[i].id].getCorners3D();
