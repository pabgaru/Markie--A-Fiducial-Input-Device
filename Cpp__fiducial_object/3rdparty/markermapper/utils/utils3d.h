...
namespace aruco_mm {
class se3;
cv::Mat solvePnP(const  std::vector<general_marker::GeneralMarker> &markers,    MarkerSet & MarkerSet,const aruco::CameraParameters &camp,bool ransac=false,cv::Mat initial_rt=cv::Mat());
...
MARKERMAPPER_API void savePCDFile(std::map<int, int> markersConfiguration, general_marker_detector::GeneralMarkerDetector *detector, std::string fpath, const MarkerSet &ms, int origin=-1) ;


MARKERMAPPER_API void savePCDFile(std::map<int, int> markersConfiguration, general_marker_detector::GeneralMarkerDetector *detector, std::string fpath, const MarkerSet &ms, const FrameSet &fset, int origin=-1, bool writeFrameIds=false, cv::Scalar marker_color=cv::Scalar(255,0,0), cv::Scalar frame_color=cv::Scalar(0,255,0)) ;
